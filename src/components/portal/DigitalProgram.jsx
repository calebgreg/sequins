        ...
        >
            ...
            {!isExpanded && (
                <div 
                    ...
                    className="bg-white rounded-[32px] p-8 shadow-sm hover:shadow-md transition-all cursor-pointer border border-stone-100 group relative overflow-hidden"
                >
                    <div className="flex items-start justify-between mb-6">
                        <div>
                            ...
                            <h3 className="font-serif text-3xl text-[#1c1c1c] mb-2">{performance.title}</h3>
                            <p className="text-gray-500 font-serif italic">Tap to view your personalized program</p>
                        </div>
                        <ChevronDown className="w-6 h-6 text-gray-400 group-hover:text-rose-500 transition-colors" />
                    </div>
                    ...
                    <div className="flex flex-wrap items-center gap-4 text-sm text-gray-500">
                        ...
                        <span className="ml-auto bg-rose-50 text-rose-600 px-3 py-1 rounded-full text-xs font-semibold">
                    ...
                    <div className="p-8 pb-6 border-b border-stone-100">
                        <div className="flex items-start justify-between mb-6">
                            <div>
                                ...
                                <h2 className="font-serif text-4xl text-[#1c1c1c] mb-2">{performance.title}</h2>
                                <div className="flex flex-wrap items-center gap-4 text-gray-500 font-serif italic text-lg">
                                    <span className="flex items-center gap-2">
                                    ...
                                    return (
                                        <div key={routine.id} className="flex items-center gap-3">
                                            ...
                                            <div className="flex-1">
                                                <div className="font-bold text-[#1c1c1c]">{routine.title}</div>
                                                {routine.song_title && (
                                                    <div className="text-sm text-gray-500 italic font-serif">"{routine.song_title}"</div>
                                                )}
                                            </div>
                                ...
                                {routinesWithTiming.map((routine, idx) => (
                                    ...
                                    >
                                        <div className="flex items-center gap-3 min-w-[60px]">
                                            ...
                                            <div className={`font-semibold ${routine.isFamilyRoutine ? 'text-rose-600' : 'text-gray-400'}`}>
                                                {idx + 1}.
                                            </div>
                                        </div>
                                        
                                        <div className="flex-1">
                                            <div className={`font-semibold ${routine.isFamilyRoutine ? 'text-[#1c1c1c]' : 'text-gray-600'}`}>
                                                {routine.title || 'Untitled Routine'}
                                            </div>
                                            {routine.song_title && (
                                                <div className="text-sm text-gray-500 italic font-serif mt-0.5">"{routine.song_title}"</div>
                                            )}
