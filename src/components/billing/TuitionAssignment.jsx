...
export default function TuitionAssignment({ isOpen, onOpenChange }) {
  ...
  const handleBulkAssign = async (targetPlanId) => {
      // Logic for bulk assign would go here (e.g. assign all unassigned to a plan)
      // For now, we'll keep it per-row for clarity, or "Auto Assign based on Level"
  };
    ...
    <div className="space-y-6">
       <div className="bg-white p-6 rounded-[32px] shadow-sm border border-gray-100">
          <div className="flex justify-between items-center mb-6">
             <div>
                <h2 className="font-serif text-2xl text-[#333333]">Student Assignments</h2>
                <p className="text-gray-400 text-sm">Link students to their billing plans.</p>
             </div>
             <div className="flex gap-2">
                <Badge variant="secondary" className="bg-red-50 text-red-500 hover:bg-red-100">
                   {stats.unassigned} Unassigned
                </Badge>
                <Badge variant="secondary" className="bg-green-50 text-green-600 hover:bg-green-100">
                   {students.length - stats.unassigned} Active
                </Badge>
             </div>
          </div>
                ...
                {filteredStudents.map(student => (
                   <div key={student.id} className="bg-white p-4 rounded-xl flex items-center justify-between shadow-sm group">
                      ...
                      <div className="w-64">
                         ...
                         >
                           <SelectTrigger className={`rounded-lg border-transparent bg-gray-50 focus:bg-white transition-colors ${!student.tuition_plan_id ? 'text-red-400' : 'text-[#333333]'}`}>
                              <SelectValue placeholder="Select Plan" />
                           </SelectTrigger>
                           <SelectContent>
                              <SelectItem value="unassigned">No Plan Assigned</SelectItem>
                              {plans.map(p => (
